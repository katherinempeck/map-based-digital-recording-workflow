...
def get_most_recent_folder(survey_data_folder:str) -> str:
    ...
    survey_data_folder : str
        Folder path to folder containing survey data subfolders, labeleld by date 
...
def organize_data_to_site_folders(survey_data_folder:str, extracted_data_folder:str, output_crs:str):
    ...
    for d in data_list:
        for layername in fiona.listlayers(d):
            ...
            if geopkg.empty:
                ...
                geopkg['original_layer_name'] = f'{layername}___{d.split("/")[-1]}' #Add three ___ between names to make it possible to input files with _ in them
    ...
    #Make another folder for photos (this will be used later in the procedure)
    ...
    query_info = []
    os.mkdir(f'{extracted_data_folder}/Other_spatial_data')
    for g in gpkg_list:
        og_name = list(g['original_layer_name'].values)[0]
        og_gpkg = og_name.split(".")[0]
        og_gpkg = og_gpkg.split("___")[1] #split based on three _ to make it possible to input files with _ in them
        og_layer = og_name.split(".")[0]
        og_layer = og_layer.split("___")[0] #split based on three _ to make it possible to input files with _ in them
        try:
        ...
        if c.empty:
            if not isinstance(g, gpd.GeoDataFrame):
                ...
                g.to_file(f'{extracted_data_folder}/Other_spatial_data/{og_name}.gpkg')
            # pass
        else:
    ...
    for fo in os.listdir(extracted_data_folder):
        if os.path.isdir(f'{extracted_data_folder}/{fo}'):
            for f in os.listdir(f'{extracted_data_folder}/{fo}'):
                #Find site forms
                if any(n in f.lower() for n in ("form", "update", "shovel", "stp")): #TODO added shovel, stp to test
                    #Write gpkg to csv
...
def blob_to_image(database:str, fid:int, outpath:str, image_label = ''):
    """A function for extracting a single image from the BLOB column in an Avenza-exported GPKG.
...
def extract_photos_and_logs(survey_data_folder, extracted_data_folder):
    ...
    for fo in os.listdir(extracted_data_folder):
        if os.path.isdir(f'{extracted_data_folder}/{fo}/Photos'):
            ...
            for index, im in enumerate(os.listdir(photo_subfolder)):
                if im.endswith('.jpg'):
                    att_list = im.split('--')
                    # placemark = att_list[0]
                    old_file_name = im #Using the actual file path name as the photo name
                    if foldername == 'None':
                        new_file_name = f'NoSiteID_IMG_{index + 1}.jpg'
                    else:
                        new_file_name = f'{foldername}_IMG_{index + 1}.jpg'
                    layer = att_list[1]
                    gpkg = att_list[2]
                    #Sometimes, orientation is not defined
                    #This should be using "NoOrientation" as the orientation in the spreadsheet but it's not
                    ...
                    if len(desc_col) > 1:
                        desc_list = []
                        for c in desc_col:
                            if relevant_row[c].values[0] is None:
                                pass
                            else:
                                desc_list.append(relevant_row[c].values[0])
                        desc = ' '.join(desc_list)
                        # desc = relevant_row[desc_col[1]]
                    elif len(desc_col) == 1:
                        desc = relevant_row[desc_col[0]].values[0]
                    if len(desc) == 0:
                        desc = 'No description recorded'
                    else:
                        pass
                    os.rename(f'{extracted_data_folder}/{foldername}/Photos/{old_file_name}', f'{extracted_data_folder}/{foldername}/Photos/{new_file_name}')
                    photo_log_data.append([new_file_name, foldername, desc, orientation])
            df = pd.DataFrame(photo_log_data, columns = ['Photo File Name', 'Site or Isolate ID', 'Description', 'Photo Orientation'])
            # print(f'Creating photo log {extracted_data_folder}/{fo}/Photos/{fo}_Photo_log.csv')
            df.to_csv(f'{extracted_data_folder}/{fo}/Photos/{fo}_Photo_log.csv')
    photo_log_data = []
    for index, ph in enumerate(os.listdir(f'{extracted_data_folder}/Other_photos')):
        if ph.endswith('.jpg'):
            att_list = ph.split('--')
            old_file_name = ph
            new_file_name = f'OtherPhotos_IMG_{index + 1}.jpg'
            layer = att_list[1]
            gpkg = att_list[2]
            orientation = att_list[3]
            fid = att_list[-1].split('.')[0]
            ...
            if len(desc) == 0:
                ...
                pass
            os.rename(f'{extracted_data_folder}/Other_photos/{old_file_name}', f'{extracted_data_folder}/Other_photos/{new_file_name}')
            photo_log_data.append([new_file_name, desc, orientation, layer, att_list[0]])
    df = pd.DataFrame(photo_log_data, columns = ['Photo File Name', 'Description', 'Photo Orientation', 'Original Layer', 'Original Name'])
    df.to_csv(f'{extracted_data_folder}/Other_photos/Photo_log.csv')


def site_form_to_text(extracted_data_folder, output_crs, supplement = False):
    #Note: supplement flag anticipating future "supplement" function to add PLSS data, etc.
    ...
    for fo in os.listdir(extracted_data_folder):
        if os.path.isdir(f'{extracted_data_folder}/{fo}'):
            ...
            if os.path.isdir(form_folder):
                ...
                for form in forms:
                    if form.endswith('.csv'):
                        if supplement == True:
                            if 'supplemented' in form:
                ...
                #What CRS to use to transform these?
